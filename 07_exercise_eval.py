...
def validate_dataset(dataset):
    ...
    for i, item in enumerate(dataset):
        ...
        if item.get("format") not in valid_formats:
            ...
            return False
        if not isinstance(item.get("solution_criteria"), str) or not item["solution_criteria"].strip():
            print(f"Dataset item {i} is missing a valid 'solution_criteria' field: {item!r}")
            return False
...
[
    {
        ...
        "solution_criteria": "Specific, checkable criteria that a correct solution must satisfy"
    },
    ...additional
]
```
...
* For each task, include a "solution_criteria" field describing the specific, concrete requirements a correct solution must meet (e.g. required fields/keys, edge cases to handle, expected inputs/outputs, naming or formatting rules). This will be used later to grade solutions, so make it precise and checkable rather than vague.
...
Evaluate the solution specifically against the solution criteria above, in addition to general code quality and correctness for the task.

Output Format
Provide your evaluation as a structured JSON object with the following fields, in this specific order:
- "strengths": An array of 1-3 key strengths
- "weaknesses": An array of 1-3 key areas for improvement
- "reasoning": A concise explanation of your overall assessment, referencing how well the solution meets the solution criteria
